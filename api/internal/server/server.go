...
func Run() error {
	...
	certStore, err := pki.NewTenantCertStore(cfg.K8sNamespace, root)
	if err != nil {
		return fmt.Errorf("tenant cert store: %w", err)
	}

	srv := &http.Server{
		Addr:    cfg.Addr,
		Handler: corsMiddleware(cfg.CorsAllowedOrigins, routes(agent, oc, rdb, verifier, db, exportStore, certStore)),
	}
	internalSrv := &http.Server{
		Addr:    cfg.MQAuthAddr,
		Handler: internalRoutes(rdb),
	}
	...
	go func() {
		log.Printf("mqauth listening on %s", internalSrv.Addr)
