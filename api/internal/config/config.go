...
func Load() (Config, error) {
	...
	cfg := Config{
		...
		RabbitMQMgmtURL:    requireEnv("RABBITMQ_MGMT_URL", &missing),
		RabbitMQMgmtUser:   requireEnv("RABBITMQ_MGMT_USER", &missing),
		RabbitMQMgmtPass:   requireEnv("RABBITMQ_MGMT_PASS", &missing),
	}
	if len(missing) > 0 {
