...
func HandleCreate(agent *iotagent.Agent, oc *orion.Client, rdb *redis.Client) http.HandlerFunc {
...
func HandleDelete(agent *iotagent.Agent, oc *orion.Client, rdb *redis.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		privs, ok := auth.PrivilegesFromContext(r.Context())
		if !ok || !privs.Has("urn:dk:kombit:gps-connector:write") {
			http.Error(w, "forbidden", http.StatusForbidden)
